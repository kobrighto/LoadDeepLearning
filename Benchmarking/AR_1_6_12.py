...
def regress(trainList,lag):
    ...
    if lag<=0:
        ...
        ar_res = ar_mod.fit(maxlag=lag,ic='aic')
    ar_res = ar_res.params
    lagList.append(ar_res)
    print('length:', len(ar_res), 'length lagList:', len(lagList))
    return ar_res
...
dirPath = '/home/minh/Desktop/Google_Data/final/AutoRegression1-6-12(first_500)(maxlag)'
chdir(dirPath)
...
for i in xrange(len(sample_moments)):
    realvalueList, predictList = autoRegression(lineNumber=int(sample_moments[i]), meanLoad=30, trainingPercent=0.9, trainingStep=1,
                                                inputvector=(1,6), labelvector=(1,12), lag=50)
...
with open('lagInfo.csv','wb') as f:
    writer = csv.writer(f)
    writer.writerows(lagList)